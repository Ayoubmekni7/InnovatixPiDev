 ...
 
{
    ...
    public function ajoutercredit(ManagerRegistry $doctrine, Request $request, SluggerInterface $slugger): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            return $this->redirectToRoute('app_ajoutercredit');
        }
...
public function approuvercredit(MailerInterface $mailer,CreditRepository $creditRepository,$id, Mailing $mailing): Response
{
    $credit = $creditRepository->find($id);
...
#[Route('/refusercredit', name: 'app_refusercredit')]
public function refusercredit(MailerInterface $mailer): Response
{
    $to = "ahmed.marzougui@esprit.tn";
    $subject = "Email";
    $content = 'Après une évaluation approfondie de votre dossier, nous regrettons de vous informer que votre demande na pas été approuvée dans sa forme actuelle.';
    $email = (new Email())
    ->from('yesser.khaloui@etudiant-fst.utm.tn')
    ->to($to)
    //->cc('cc@example.com')
